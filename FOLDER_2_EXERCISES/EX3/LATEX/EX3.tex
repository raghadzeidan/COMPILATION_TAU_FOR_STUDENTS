...
The condition of if and while statements is the primitive type int.
\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
RioMare defines four kinds of scopes: block scopes of if and while statements,
function scopes, class scopes and the outermost global scope.
...
Note that arrays and classes can only be defined
in the outermost (global) scope,
so this sub section is really about functions and variables.
Functions can be defined only in the class scopes, and the global scope.
...
only the global scope is searched for its declaration. Resolving a variable identifier
