...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\date{Due 31/1/2019 before 0700}
...
building an entire compiler for Poseidon programs.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Programming Assignment}
The fourth (and last) exercise implements the code generation phase for Poseidon programs.
...
The input for this last exercise is a (single) text file, containing a Poseidon
...
\paragraph{Integers} in Poseidon are artificially bounded between $-2^{15}$ and $2^{15}-1$.
...
and to distinguish Poseidon operators from the usual arithmetic signs,
we shall use a Poseidon subscript inside brackets:
($*_{[Poseidon]}$, $+_{[Poseidon]}$ etc.)
\begin{table}[h]
\centering
...
\\ \hline
\end{tabular}
\caption{Semantics of Poseidon binary operations between integers
...
Every (valid) Poseidon program has a main function with signature: \verb"void main()".
